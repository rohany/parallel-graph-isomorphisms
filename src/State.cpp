...
{
  return r_pred(n, m) && r_succ(n, m) && r_in(n, m) && r_out(n, m) && r_new(n, m);
}
...
std::vector<std::pair<int, int>> State::generateCandidatePairs()
{
  ...
  auto filtered = std::remove_if(possiblePairs.begin(), possiblePairs.end(), [&](std::pair<int, int> x) {
    return !F_pred(x.first, x.second);
  });
  // filtered array
